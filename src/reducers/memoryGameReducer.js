...
export function gameReducer(state, action) {
  switch (action.type) {
    ...
    case "EVALUATE_FLIPPED": {
      ...
      const logEntry = createLogEntry(turns, isMatch);

      // Start of game-over calculation
      ...
      const shouldEnd = allMatched || outOfTurns || noChanceToWin;

      if (shouldEnd) {
        const feedback = allMatched
          ? {
